...
var crossbeamsGridEvents = {
  ...
  promptClick: function(target) {
    ...
    if (confirm(prompt)) {
      ...
      //window.location = url;
      //document.body.innerHTML += '<form id="dynForm" action="' + url + '" method="post"><input type="hidden" name="q" value="a"></form>';
      document.body.innerHTML += '<form id="dynForm" action="' + url + '" method="post"></form>';
